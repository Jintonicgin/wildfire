...
def make_precip_features(lat, lng, end_dt, periods=[7, 14, 30, 60, 90]):
    ...
    sorted_precip = sorted(precip_data.items(), key=lambda item: datetime.datetime.strptime(item[0], '%Y%m%d'), reverse=True)
    ...
    if 'T2M' in df_weather.columns and 'RH2M' in df_weather.columns:
        ...
        if len(t_vals) > 0 and len(rh_vals) > 0:
            ...
            stats_features['temp_humidity_correlation'] = float(np.corrcoef(t_vals[-min(len(t_vals), len(rh_vals)):], 
                                                                           rh_vals[-min(len(t_vals), len(rh_vals)):])[0,1])
...
def fetch_all_weather_features(lat, lon, timestamp, offset_days=0):
    ...
    all_hourly_data = []
    for i in range(8):
        daily_data = _fetch_nasa_raw_hourly_data_for_day(lat, lon, (end_dt - datetime.timedelta(days=i)).strftime("%Y%m%d"))
    ...
    if any(pd.isna(v) for v in fwi_inputs.values()):
        ...
        fwi = fwi_calc(T=fwi_inputs["T2M"], RH=fwi_inputs["RH2M"], W=fwi_inputs["WS10M"], P=fwi_inputs["PREC"], month=end_dt.month)
        print("✅ FWI 지수 계산 완료")
