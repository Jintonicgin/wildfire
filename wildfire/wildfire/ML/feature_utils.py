...
def generate_statistical_features(weather_features):
    ...
    for param in weather_params:
        ...
        if all_values:
            ...
            stats_features[f"{param}_std_past"] = (sum([(x - stats_features[f"{param}_mean_past"]) ** 2 for x in
                                                        all_values]) / len(all_values)) ** 0.5
        else:
        ...
        if day_values:
            ...
            stats_features[f"{param}_std_24h_past"] = (sum([(x - stats_features[f"{param}_mean_24h_past"]) ** 2 for x in
                                                            day_values]) / len(day_values)) ** 0.5
        else:
            for stat in ['max', 'min', 'mean', 'std']:
...
def generate_custom_features(weather_features, gee_features):
    ...
    custom_features['terrain_complexity_index'] = float(
        (slope_std / max(slope_mean, 1)) + (elevation_std / max(elevation_mean, 1)))
    ...
    if len(wind_values) > 3:
        ...
        custom_features['wind_strength_consistency'] = float(
            wind_mean * (1 - wind_std / wind_mean) if wind_mean > 0 else 0)
    else:
