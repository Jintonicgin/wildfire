...
MODEL_PATH = "wildfire/ML/"

# --- 모델 및 스케일러 로드 ---
MODELS = {
    "area_model": joblib.load(MODEL_PATH + "area_regressor_model_v3_tuned.joblib"),
    "speed_model": joblib.load(MODEL_PATH + "speed_classifier_model_v2_tuned_cw.joblib"),
    "direction_model": joblib.load(MODEL_PATH + "direction_classifier_model_v2_tuned_cw.joblib"),
    "area_scaler": joblib.load(MODEL_PATH + "area_model_scaler_v3_tuned.joblib"),
    "speed_scaler": joblib.load(MODEL_PATH + "speed_model_scaler_v2_tuned_cw.joblib"),
}
with open(MODEL_PATH + "area_model_columns_v3_tuned.json") as f:
    MODELS["area_cols"] = json.load(f)
with open(MODEL_PATH + "speed_model_columns_v2_tuned_cw.json") as f:
    MODELS["speed_cols"] = json.load(f)
with open(MODEL_PATH + "direction_model_columns_v2_tuned_cw.json") as f:
